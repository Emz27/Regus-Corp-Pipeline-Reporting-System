...
$year1 = $_POST['year1_project_owner'];
$year2 = $_POST['year2_project_owner'];
...
$openedMonth_result = $mysqli->query('select month(open_date) as month, count(*) as open, year(open_date) as year
   from center inner join user on user.id = center.project_owner
    where year(open_date) = year("'.$year1."-1-1".'") and cancel_date is null and project_owner = "'.$project_owner.'"
    group by month
   order by month asc');
...
$openedMonth_result = $mysqli->query('select month(open_date) as month, count(*) as open, year(open_date) as year
    from center
     where year(open_date) = year("'.$year2."-1-1".'") and cancel_date is null and project_owner = "'.$project_owner.'"
     group by month
    order by month asc');
while($openedMonth_row = $openedMonth_result->fetch_assoc()){
  ...
  $project_owner = $mysqli->query("select * from user where id ='".$project_owner."'");

  $row = $project_owner->fetch_assoc();

  $data['project_owner'] = $project_owner;
  $data['project_owner_name'] = $row['firstname']." ".$row['lastname'];
  $data['year1'] = $year1;
  $data['year2'] = $year2;
  ...
  $data['table'] = '
    <table class="table-bordered col-md-12">
      <thead>
        <tr><th colspan=3 class="text-center">'.$data['project_owner_name'].'</th></tr>
