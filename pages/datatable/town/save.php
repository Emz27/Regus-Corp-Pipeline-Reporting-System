...
if($crud=='N'){
	...
	$stmt = $mysqli->prepare("insert into town( description,city)
		VALUES( ? , ? )");

	$stmt->bind_param("ss", $description,$city);
	...
	$stmt = $mysqli->prepare("update town set description = ?, city = ? where id = ?");


	$stmt->bind_param("sss", $description,$city,  $id);
