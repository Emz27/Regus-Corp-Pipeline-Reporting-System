...
if($crud=='N'){
	...
	$stmt = $mysqli->prepare("insert into city( description,region)
		VALUES( ? , ? )");

	$stmt->bind_param("ss", $description,$region);
	...
	$stmt = $mysqli->prepare("update city set description = ?, region = ? where id = ?");


	$stmt->bind_param("sss", $description, $region, $id);
