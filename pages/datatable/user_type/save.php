...
if($crud=='N'){
	...
	$stmt = $mysqli->prepare("insert into user_type( num, description, level)
		VALUES( ? , ? , ?)");

	$stmt->bind_param("isi",$num, $description, $level);
	...
	$stmt = $mysqli->prepare("update user_type set num = ?, description = ?, level = ? where id = ?");


	$stmt->bind_param("ssss",$num, $description, $level, $id);
